...
func UpdateAnimal(w http.ResponseWriter, r *http.Request) {
	...
	queryCheck := `SELECT COUNT(*) FROM animals WHERE id = $1`
	var count int
	err = db.QueryRow(queryCheck, id).Scan(&count)
	if err != nil {
		http.Error(w, "Could not check animal", http.StatusInternalServerError)
		return
	}

	if count > 0 {
		queryUpdate := `UPDATE animals SET name = $1, class = $2, legs = $3 WHERE id = $4`
		_, err := db.Exec(queryUpdate, animal.Name, animal.Class, animal.Legs, id)
		if err != nil {
